...
@Component({
  selector: 'ngx-firmaelgamal',
  styleUrls: ['./firmaelgamal.component.scss'],
  templateUrl: './firmaelgamal.component.html',
})
export class FirmaElGamalComponent {
  ...
  constructor(private dialogService: NbDialogService) {
    this.minNumber = 65536;
    this.maxNumber = 9000000;
  ...
  verifyButton(shaTxt, keyTxt, signTxt) {
    ...
    if (keyTxt.length % 5 != 0 || signTxt.length % 5 != 0) {
      this.showClaveIncorrecta();
      return;
    }
    var key = this.base64ToArr(keyTxt, 5);
  ...
  sign(numb, p, alpha, a) { // k entre 1 y q
    let ranK;
    let gamma = BigInt(0);
    let delta = 0;
    while (gamma == BigInt(0) || delta == 0) {
      /*
      ranK = getRandomInt(1, q - 1);
      gamma = power(alpha, ranK, p) % q;
      delta = ((numb + a * gamma) * modInverse(ranK, q)) % q;
      */
      ranK = this.getRandomInt(2, p - 2);
      while (this.gcd(ranK, p - 1) != 1) ranK = this.getRandomInt(2, p - 2);
      gamma = this.powerBigInt(BigInt(alpha), BigInt(ranK), BigInt(p));
      let deltaBig = ((BigInt(numb) - BigInt(a) * gamma) * BigInt(this.modInverse(ranK, p - 1))) % BigInt(p - 1);
  ...
  verify(numb, gamma, delta, p, alpha, beta) {
    ...
    return this.powerBigInt(BigInt(alpha), BigInt(numb), BigInt(p)) == (this.powerBigInt(BigInt(beta), BigInt(gamma), BigInt(p)) * this.powerBigInt(BigInt(gamma), BigInt(delta), BigInt(p))) % BigInt(p);
  }

  signSha(shaString, p, alpha, a) {
    let arr = [];
    let signArr = [];
    let jump = 4;
    let baseSep = 5;
    ...
    for (let i = 0; i < arr.length; i++) {
      signArr.push(this.sign(arr[i], p, alpha, a));
    }
    //console.log(signArr);
    // Turn to base64
    var signText = "";
    for (let i = 0; i < signArr.length; i++) {
      signText += this.Base64.fromNumber(signArr[i][0], baseSep) + this.Base64.fromNumber(signArr[i][1], baseSep);
    }
    return signText;
  }

  verifySha(shaString, signStr, p, alpha, beta) {
    let arr = [];
    let jump = 4;
    let signArr = [];
    let auxArr = [];
    ...
    for (var i = 0; i < signStr.length / baseSep; i += 2) {
      auxArr.push(this.Base64.toNumber(signStr.substring(baseSep * i, baseSep * (i + 1))));
      auxArr.push(this.Base64.toNumber(signStr.substring(baseSep * (i + 1), baseSep * (i + 2))));
      signArr.push(auxArr);
      auxArr = [];
    }
    //console.log(signArr);
    for (let i = 0; i < arr.length; i++) {
      if (!this.verify(arr[i], signArr[i][0], signArr[i][1], p, alpha, beta)) return false;
    }
    return true;
  }
  ...
  generateKey() {
    let key = [];
    let ranNum = this.getRandomInt(0, this.pArrLen);
    let p = this.pArr[ranNum];
    //p
    key.push(p);
    ranNum = this.getRandomInt(2, p - 1);
    let alpha = ranNum;
    ranNum = this.getRandomInt(2, p - 1);
    let a = ranNum;
    let beta = this.power(alpha, a, p);
    key.push(alpha);
    key.push(beta);
    key.push(a);
    return key;
